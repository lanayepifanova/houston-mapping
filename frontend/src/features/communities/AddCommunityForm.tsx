...
export const AddCommunityForm = () => {
  ...
  return (
    <div className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm space-y-3">
      <h2 className="text-xl font-semibold">Add community</h2>
      <p className="text-xs text-slate-500">Tags: use commas to separate (e.g., coworking, events, mentorship).</p>
      <form onSubmit={handleSubmit} className="grid gap-3 sm:grid-cols-2 md:grid-cols-3">
        ...
        <button
          type="submit"
          className="rounded-lg bg-amber-600 px-4 py-2 text-white shadow hover:bg-amber-700 sm:col-span-2 md:col-span-3"
          disabled={mutation.isPending}
        >
          {mutation.isPending ? "Saving…" : "Add community"}
        </button>
      </form>
    </div>
  );
